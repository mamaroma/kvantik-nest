(() => {
    ...
    const initPerfMeter = () => {
        ...
        const serverMeter = document.getElementById('serverPerfInfo');
        const serverMs = serverMeter?.dataset?.serverElapsedMs;
        const serverText = serverMs ? ` • На сервере: <strong>${Number(serverMs).toFixed(2)}</strong> мс.` : '';

        meter.innerHTML = `Время загрузки страницы: <strong>${sec}</strong> сек.${serverText}`;
        if (serverMeter) {
            serverMeter.innerHTML = 'Заголовок ответа X-Elapsed-Time также доступен для REST API и GraphQL-запросов.';
        }
    };
